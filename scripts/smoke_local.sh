...
QVIS_PY="$ROOT_DIR/query-visualization/.venv/bin/python"
T2S_PY="$ROOT_DIR/text-to-sql/backend/.venv/bin/python"
...
(
  set -a
  if [ -f "$ROOT_DIR/text-to-sql/.env" ]; then
    # shellcheck disable=SC1091
    . "$ROOT_DIR/text-to-sql/.env"
  fi
  set +a
  cd "$ROOT_DIR/text-to-sql/backend"
...
(
  set -a
  if [ -f "$ROOT_DIR/query-visualization/.env" ]; then
    # shellcheck disable=SC1091
    . "$ROOT_DIR/query-visualization/.env"
  fi
  set +a
  cd "$ROOT_DIR/query-visualization"
