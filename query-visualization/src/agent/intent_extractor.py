...
from src.config.llm_config import OPENAI_API_KEY, OPENAI_MODEL
from src.utils.logging import log_event


def _find_column_in_query(user_query: str, columns: List[str]) -> str | None:
    # 질문에 컬럼명이 포함되어 있으면 우선 사용
    q = user_query.lower()
    for col in columns:
        if col.lower() in q:
            return col
...
def _pick_numeric_column(dtypes: Dict[str, str]) -> str | None:
...
def extract_intent(user_query: str, df_schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        primary_outcome = llm_result.y or llm_result.x or _pick_numeric_column(dtypes)
        time_var = llm_result.x if analysis_intent == "trend" else None
        group_var = llm_result.group_by

        log_event("intent.llm.success", llm_result.model_dump(exclude_none=True))
        ...
        primary_outcome = _find_column_in_query(user_query, columns) or _pick_numeric_column(dtypes)
        time_var = _pick_time_column(columns) if analysis_intent == "trend" else None
